...
Copy your desired variant to the rofi configuration file.

```sh
cp rose-pine.rasi ~/.config/rofi/config.rasi

# Or
cp rose-pine-moon.rasi ~/.config/rofi/config.rasi
cp rose-pine-dawn.rasi ~/.config/rofi/config.rasi
```
...
Modify `template.rasi` using Rosé Pine variables, then build variants:

```sh
npx @rose-pine/build@0.5.1 -t template.rasi -o ./
```
